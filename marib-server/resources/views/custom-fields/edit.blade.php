...
@section('script')
<script>
    // لا حاجة لأي أكواد color_values هنا.
    // دالة الاسترجاع بعد نجاح التحديث:
...
<section class="section">
    ...
    <form action="{{ route('custom-fields.update', $custom_field->id) }}"
          class="edit-form"
          data-success-function="afterCustomFieldUpdate"
          method="POST"
                ...
                <div class="card">
                    ...
                    <div class="card-body mt-3">
                        <div class="row">
                            ...
                            <!-- النوع (ثابت أثناء التعديل) -->
                            <div class="col-md-12 form-group mandatory">
                                <label for="type" class="mandatory form-label">{{ __('Field Type') }}</label>
                                <select name="type" id="type" class="form-select form-control" disabled>
                                    <option value="{{ $custom_field->type }}" selected>
                                        {{ ucfirst($custom_field->type) }}
                                    </option>
                                </select>
                                <!-- لو أردت السماح بتغيير النوع، احذف disabled واملأ بقية الأنواع -->
                                <input type="hidden" name="type" value="{{ $custom_field->type }}">
                            </div>

                            <!-- قيم الحقل: فقط للأنواع القائمة على خيارات -->
                            @if(in_array($custom_field->type, ['radio','checkbox','dropdown']))
                                <div class="col-md-12">
                            ...
                            <!-- Min/Max: number | textbox | color -->
                            @if(in_array($custom_field->type, ['textbox','number','color']))
                                <div class="col-md-6 form-group">
                                    ...
                                    <div class="input_hint">
                                        {{ __("This will be applied only for") }}:
                                        <text class="highlighted_text">{{ __("text") }}, {{ __("number") }}, {{ __("color") }}</text>.
                                    </div>
                                </div>
                                <div class="col-md-6 form-group">
                            ...
                            <div class="col-md-12 form-group">
                                ...
                                <textarea name="notes" id="notes" class="form-control" rows="3">{{ $custom_field->notes }}</textarea>
                            </div>
                            ...
                            <div class="row mt-3">
                                <div class="col-md-6 form-group mandatory">
                                    <div class="form-check form-switch">
                                        ...
                                        <input class="form-check-input status-switch" type="checkbox" role="switch" aria-label="required" {{ $custom_field->required ? 'checked' : '' }}>
                                        {{ __('Required') }}
                                ...
                                <div class="col-md-6 form-group mandatory">
                                    <div class="form-check form-switch">
                                        ...
                                        <input class="form-check-input status-switch" type="checkbox" role="switch" aria-label="status" {{ $custom_field->status ? 'checked' : '' }}>
                                        {{ __('Active') }}
                            ...
                            <div class="row mt-3">
                                <div class="col-md-6 form-group">
                                    <div class="form-check form-switch">
                                        <input type="hidden" name="is_customer_option" value="0">
                                        <input class="form-check-input"
                                               type="checkbox"
                                               role="switch"
                                               name="is_customer_option"
                                               id="is_customer_option"
                                               value="1"
                                               {{ $custom_field->is_customer_option ? 'checked' : '' }}>
                                        <label class="form-check-label" for="is_customer_option">{{ __('خيار للزبون') }}</label>
                                        <small class="form-text text-muted">
                                            {{ __('إذا كان محددًا، سيظهر هذا الحقل كخيار للزبائن (مثل اللون يختاره المستخدم من التطبيق)') }}
                                        </small>
                                    </div>
                                </div>
                            </div>
    ...
    <!-- جدول القيم وإدارة الخيارات: فقط للأنواع القائمة على خيارات -->
    @if(in_array($custom_field->type, ['radio','checkbox','dropdown']))
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <form action="{{ route('custom-fields.value.add', $custom_field->id) }}"
                            ...
                            @csrf
                            <div class="col-md-12 form-group mandatory">
                                <label for="values_input_modal" class="mandatory form-label">{{ __('Field Values') }}</label>
                                <input type="text" name="values" id="values_input_modal" class="form-control" value="{{ old('values') }}" data-parsley-required="true">
                            </div>
        ...
        {{-- edit modal --}}
        <div id="editModal" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel2" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="myModalLabel2">{{ __('Edit Custom Field Values') }}</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
...
@section('js')
<script>
    // ربط سويتشات required/status بقيم hidden inputs
    document.addEventListener('change', function (e) {
        if (e.target && e.target.classList.contains('status-switch')) {
            const inputHidden = e.target.closest('.form-check').querySelector('input[type="hidden"]');
            if (inputHidden) inputHidden.value = e.target.checked ? '1' : '0';
        }
    });
</script>
@endsection
