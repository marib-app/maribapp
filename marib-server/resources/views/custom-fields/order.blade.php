...
@section('title')
    {{__("Change Custom Fields Order")}} - {{ $category->name }}
@endsection
...
@section('content')
    <section class="section">
        <div class="buttons">
            <a class="btn btn-primary" href="{{ route('category.custom-fields', $category->id) }}">< {{__("Back to Custom Fields")}} </a>
        </div>
        <div class="row">
            <div class="col-md-12 grid-margin stretch-card">
                <div class="card">
                    <div class="card-body">
                        <form class="pt-3" id="update-custom-fields-order-form" action="{{ route('custom-fields.order.update')}}" method="POST" novalidate="novalidate">
                            @csrf
