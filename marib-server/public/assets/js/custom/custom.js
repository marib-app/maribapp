...
$(document).on('click', '.view-more', function (e) {
    ...
    let fullDescription = row.find('.full-description');
    let shortDescription = row.find('.short-description');


    console.log(fullDescription,fullDescription.is(':hidden'))
    if (fullDescription.is(':hidden')) {
        fullDescription.hide();
        shortDescription.show();
        $this.text(trans('View Less'));
    } else {
        fullDescription.show();
        shortDescription.hide();
        $this.text(trans('View More'));
    }
});
...
$("#update-custom-fields-order-form").on("submit", function (e) {
    e.preventDefault();
