...
// منطقي فقط
import 'package:marib/ui/screens/cart/adress_ui.dart';
...
class _AdressScreenState extends State<AdressScreen> {
  ...
  Future<void> _loadAddresses() async {
    ...
    try {
      ...
      if (!mounted) return;
      final dynamic message = error.payload is Map<String, dynamic>
  ...
  Future<void> _onAddNew() async {
    ...
    await _performMutation(() async {
      ...
      final Set<int> nextIds =
          addresses.map(_addressId).whereType<int>().toSet();
      final int? createdId = nextIds.difference(previousIds).isNotEmpty
  ...
  Future<void> _onEdit(int index) async {
    ...
    final Map<String, dynamic>? result =
        await _showAddressForm(initial: current);
    if (result == null) return;
                      ...
                      children: [
                        ...
                        Container(
                          decoration: BoxDecoration(
                            ...
                            border: Border.all(
                              color:
                                  context.color.territoryColor.withOpacity(0.4),
                            ),
                          ),
                        ...
                        UiUtils.buildButton(
                          context,
                          buttonTitle:
                              initial == null ? 'حفظ العنوان' : 'تحديث العنوان',
                          radius: 14,
                          height: 54,
                          isInProgress: saving,
                          ...
                          onPressed: () {
                            ...
                            if (lat == null || lng == null) {
                              setModalState(() {
                                locationError =
                                    'الرجاء تحديد الموقع الجغرافي قبل الحفظ.';
                              });
                              return;
                            }
  ...
  Future<Map<String, dynamic>?> _pickLocation(
    ...
    return TextFormField(
      ...
      decoration: InputDecoration(
        ...
        border: OutlineInputBorder(
          ...
          borderSide:
              BorderSide(color: context.color.borderColor.withOpacity(0.4)),
        ),
  ...
  String _describeLocation(Map<String, dynamic> location) {
    final List<String> parts = <String>[
      if ((location['formatted_address']?.toString().trim().isNotEmpty ??
          false))
  ...
  Future<bool> _performMutation(
    ...
    String? _resolveHttpError(ApiHttpException error) {
      ...
      void collectStrings(dynamic value, List<String> target) {
        ...
        if (value is Map) {
          value.forEach(
              (dynamic _, dynamic entry) => collectStrings(entry, target));
        }
      }
  ...
  void _popWithSelection() {
    ...
    final int? resolvedId = _selectedAddressId ??
        _defaultAddressId ??
  ...
  Set<int> get _knownAddressIds =>
      _addresses.map(_addressId).whereType<int>().toSet();
}
