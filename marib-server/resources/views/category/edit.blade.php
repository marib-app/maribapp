                            ...
                            <div class="row">
                                ...
                                <div class="col-md-6">
                                    <div class="col-md-12 form-group mandatory">
                                        ...
                                        <select name="parent_category_id" class="form-select form-control" id="p_category" data-placeholder="{{__("Select Category")}}">
                                            <option value="">لا يوجد قسم رئيسي (قسم رئيسي)</option>
                                            @php
                                                $categoriesCollection = collect($categories);
                                            @endphp
                                            {!! \App\Services\HelperService::childCategoryRendering($categoriesCollection, 0, null) !!}
                                        </select>
                                        <script>
                                            // Set the selected value after rendering
                                            document.addEventListener('DOMContentLoaded', function() {
                                                const parentCategoryId = '{{ $category_data->parent_category_id }}';
                                                if (parentCategoryId) {
                                                    document.querySelector('#p_category option[value="' + parentCategoryId + '"]').selected = true;
                                                }
                                            });
                                        </script>
                                    </div>
                            ...
                            @foreach($languages as $key=>$language)
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-md-12 form-group">
                                        ...
                                        <input name="translations[{{$language->id}}]" id="name" class="form-control" value="{{ $translations[$language->id] ?? '' }}">
                                    </div>
                                </div>
                            @endforeach
                        </div>
