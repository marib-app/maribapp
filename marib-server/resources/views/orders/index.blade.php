        ...
        <div class="col-md-12">
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <div class="row mb-4">
                        <div class="col-md-12">
                            <div class="card card-outline card-info">
                                <div class="card-header">
                                    <h3 class="card-title">خيارات البحث والتصفية</h3>
                    ...
                    @php
                        $ordersCollection = $orders instanceof \Illuminate\Pagination\AbstractPaginator
                            ? $orders->getCollection()
                            : collect($orders);

                        $groupByWithFallback = function ($collection, callable $callback) {
                            return $collection->groupBy(function ($order) use ($callback) {
                                $value = $callback($order);

                                return $value === null || $value === '' ? '__undefined__' : $value;
                            });
                        };

                        $deliveryStatusCounts = $groupByWithFallback($ordersCollection, function ($order) {
                            return $order->delivery_payment_status
                                ?: data_get($order->delivery_payment_summary, 'status');
                        });

                        $paymentStatusCounts = $groupByWithFallback($ordersCollection, fn ($order) => $order->payment_status);

                        $orderStatusCounts = $groupByWithFallback($ordersCollection, fn ($order) => $order->order_status);
                        ...
                        $deliveryStatusBadgeClasses = [
                            'paid' => 'bg-success',
                            'pending' => 'bg-warning',
                            'waived' => 'bg-secondary',
                            'due_on_delivery' => 'bg-info',
                            'due_now' => 'bg-info',
                            'partial' => 'bg-primary',
                            'failed' => 'bg-danger',
                        ];
                    ...
                    <div class="row mb-4">
                        <div class="col-md-4 mb-3">
                            <div class="card h-100">
                                <div class="card-header">ملخص حالة دفع التوصيل</div>
                                <div class="card-body">
                                    @forelse($deliveryStatusCounts as $status => $collection)
                                        @php
                                            $statusKey = $status === '__undefined__' ? null : $status;
                                            $deliveryStatusLabel = $statusKey
                                                ? ($deliveryPaymentStatusLabels[$statusKey]
                                                    ?? \Illuminate\Support\Str::of($statusKey)->replace('_', ' ')->headline())
                                                : 'غير محدد';
                                            $deliveryStatusClass = $statusKey
                                                ? ($deliveryStatusBadgeClasses[$statusKey] ?? 'bg-secondary')
                                                : 'bg-secondary';
                                        @endphp
                                        <div class="d-flex align-items-center mb-2">
                                            <span class="badge {{ $deliveryStatusClass }} mx-2">{{ $deliveryStatusLabel }}</span>
                                            <span class="text-muted">{{ $collection->count() }} طلب</span>
                                        </div>
                                    @empty
                        ...
                        <div class="col-md-4 mb-3">
                            <div class="card h-100">
                                <div class="card-header">ملخص حالة الدفع</div>
                                <div class="card-body">
                                    @forelse($paymentStatusCounts as $status => $collection)
                                        @php
                                            $statusKey = $status === '__undefined__' ? null : $status;
                                            $paymentStatusLabel = $statusKey
                                                ? ($paymentStatusLabels[$statusKey]
                                                    ?? \Illuminate\Support\Str::of($statusKey)->replace('_', ' ')->headline())
                                                : 'غير محدد';
                                            $paymentStatusClass = $statusKey
                                                ? ($paymentStatusBadgeClasses[$statusKey] ?? 'bg-secondary')
                                                : 'bg-secondary';
                                        @endphp
                                        <div class="d-flex align-items-center mb-2">
                                            <span class="badge {{ $paymentStatusClass }} mx-2">{{ $paymentStatusLabel }}</span>
                                            <span class="text-muted">{{ $collection->count() }} طلب</span>
                                        </div>
                                    @empty
                                        <p class="text-muted mb-0">لا توجد بيانات متاحة.</p>
                        ...
                        <div class="col-md-4 mb-3">
                            <div class="card h-100">
                                <div class="card-header">ملخص حالة الطلب</div>
                                <div class="card-body">
                                    @php
                                    ...
                                    @endphp
                                    @forelse($orderStatusCounts as $status => $collection)
                                        @php
                                            $statusKey = $status === '__undefined__' ? null : $status;
                                            $statusColor = $statusKey
                                                ? (optional($statusCollection->firstWhere('code', $statusKey))->color ?: '#6c757d')
                                                : '#6c757d';
                                        ...
                                        @endphp
                                        <div class="d-flex align-items-center mb-2">
                                            <span class="badge mx-2" style="background-color: {{ $statusColor ?? '#6c757d' }}">{{ $statusLabel ?: 'غير مسمى' }}</span>
                                            <span class="text-muted">{{ $collection->count() }} طلب</span>
                                        </div>
                                    @empty
                                        <p class="text-muted mb-0">لا توجد بيانات متاحة.</p>
                            ...
                            <tbody>
                                @forelse($orders as $order)
                                    <tr>
                                        ...
                                        <td>{{ number_format($order->final_amount, 2) }} ريال</td>

                                            @php

                                                $deliveryStatusValue = $order->delivery_payment_status
                                                    ?: data_get($order->delivery_payment_summary, 'status');
                                        ...
                                        <td>
                                            <span class="badge {{ $deliveryStatusClass }}">{{ $deliveryStatusLabel }}</span>
