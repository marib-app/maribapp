...
abstract class LoginSystem {
  ...
  void emit(MLoginState state) {
    ...
    for (final listener in List<void Function(MLoginState)>.from(listeners)) {
      listener.call(state);
    }
    onEvent(state);
  }
...
class MMultiAuthentication {
  ...
  VoidCallback listen(Function(MLoginState state) fn) {
    final List<VoidCallback> removers = [];
    systems.forEach((String key, LoginSystem value) {
      final remove = systems[key]?.addListener(fn);
      if (remove != null) {
        removers.add(remove);
      }
    });
    return () {
      for (final remover in removers.reversed) {
        remover();
      }
    };
  }
