...
class FaqController extends Controller
{
    ...
    {
        ResponseService::noAnyPermissionThenRedirect(['contact-us-list', 'contact-us-delete']);
        return view('contact-us');
    }

    /**
     * عرض قائمة رسائل "اتصل بنا"
     */
    public function contactUsShow(Request $request)
    {
        try {
            ResponseService::noPermissionThenSendJson('contact-us-list');
            $offset = $request->input('offset', 0);
            $limit = $request->input('limit', 10);
            $sort = $request->input('sort', 'id');
            $order = $request->input('order', 'ASC');

            $sql = \App\Models\ContactUs::orderBy($sort, $order);
            ...
            foreach ($result as $key => $row) {
                ...
                if (Auth::user()->can('contact-us-delete')) {
                    $operate .= BootstrapTableService::deleteButton(route('contact-us.delete', $row->id));
                }
            ...
            ResponseService::logErrorResponse($th, "FaqController --> contactUsShow");
            ResponseService::errorResponse();
        }
    }
    ...
    {
        try {
            ResponseService::noPermissionThenSendJson('contact-us-delete');
            \App\Models\ContactUs::findOrFail($id)->delete();
            ResponseService::successResponse('تم حذف الرسالة بنجاح');
        } catch (Throwable $th) {
            ResponseService::logErrorResponse($th, "FaqController -> contactUsDelete");
            ResponseService::errorResponse('حدث خطأ ما');
        }
    }
